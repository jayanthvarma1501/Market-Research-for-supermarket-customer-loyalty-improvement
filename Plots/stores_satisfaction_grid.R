## @knitr stores_satisfaction_grid
...
ggplot(d, aes(x = reorder(Sections, -value), y= round(value, digits=2), label=round(value, digits=2))) +
  geom_bar(stat='identity', aes(group = key, fill = value)) +
  #geom_line(aes(group = key), color='burlywood4', size=1.5) +
