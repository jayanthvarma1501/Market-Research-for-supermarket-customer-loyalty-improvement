...
# ggplot(d, aes(x = reorder(key, -value), y= value, label=value)) +
#   geom_bar(stat='identity', aes(group = sections, fill = value)) +
...
# 
# dataset1 %>% 
#   ggplot(aes(x = reorder(sections, -edeka), y= edeka)) +
#   geom_bar(stat = "identity") # default
# 
# p<-ggplot(data=dataset, aes(x=c('regional','eigenmarke','marken','og','bb','fleish','wurst','molke','suess','alkoholfrei'), y=regional)) +
...
# g
# my_gg <- mg + geom_point_interactive(
#   aes(tooltip = key, data_id = key), size = 2) 
